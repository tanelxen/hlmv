...
{
    mstudioseqgroup_t* pseqgroup = (mstudioseqgroup_t *)((byte *)m_pstudiohdr + m_pstudiohdr->seqgroupindex) + pseqdesc->seqgroup;
...
void StudioModel::Lighting(float *lv, int bone, int flags, vec3_t normal)
{
	...
	{
		float lightcos = DotProduct(normal, g_blightvec[bone]); // -1 colinear, 1 opposite
        lightcos = fmin(lightcos, 1.0);

		illum += g_shadelight;

        float r = fmax(g_lambert, 1.0);
...
void StudioModel::Chrome(int *pchrome, int bone, vec3_t normal)
{
	...
	// calc s coord
	float n = DotProduct( normal, g_chromeright[bone] );
...
mstudiomodel_t* StudioModel::getModel(int bodypart)
{
	...
	int index = (m_bodynum / pbodypart->base) % pbodypart->nummodels;
...
void StudioModel::DrawModel()
{
	...
	if (m_pstudiohdr->numbodyparts == 0) return;
	...
	{
        mstudiomodel_t* pmodel = getModel(i);
		DrawPart(pmodel);
	}
...
================
*/
void StudioModel::DrawPart(mstudiomodel_t* pmodel)
{
	int					i, j;
	mstudiomesh_t		*pmesh;
	byte				*pvertbone;
	...
	pvertbone = ((byte *)m_pstudiohdr + pmodel->vertinfoindex);
	pnormbone = ((byte *)m_pstudiohdr + pmodel->norminfoindex);
	ptexture = (mstudiotexture_t *)((byte *)m_ptexturehdr + m_ptexturehdr->textureindex);

	pmesh = (mstudiomesh_t *)((byte *)m_pstudiohdr + pmodel->meshindex);

	pstudioverts = (vec3_t *)((byte *)m_pstudiohdr + pmodel->vertindex);
	pstudionorms = (vec3_t *)((byte *)m_pstudiohdr + pmodel->normindex);
...
//
	...
	for (j = 0; j < pmodel->nummesh; j++)
	{
		...
		pmesh = (mstudiomesh_t *)((byte *)m_pstudiohdr + pmodel->meshindex) + j;
        ...
        bool isChrome = ptexture[pskinref[pmesh->skinref]].flags & STUDIO_NF_CHROME;
