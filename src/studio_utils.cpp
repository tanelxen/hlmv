...
void StudioModel::GetSequenceInfo(float &pflFrameRate, float &pflGroundSpeed)
{
	...
	if (pseqdesc->numframes > 1)
	{
        ...
        float x = pseqdesc->linearmovement[0];
        float y = pseqdesc->linearmovement[1];
        float z = pseqdesc->linearmovement[2];
		float speed = sqrt(x*x + y*y + z*z);
        
		pflGroundSpeed = speed * pseqdesc->fps / (pseqdesc->numframes - 1);
	}
	else
	{
		pflFrameRate = 256.0;
